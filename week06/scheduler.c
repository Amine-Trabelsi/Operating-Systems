...
void read_file(FILE* file){
    ...
    // and store theim in the array {data}
    char temp;
    fscanf(file, "%s%s%s", &temp, &temp, &temp);
    data_size = 0;
    int idx,at,bt;
    while(fscanf(file, "%d%d%d", &idx, &at , &bt) != EOF) {
        ProcessData nw; nw.idx = idx; nw.at = at; nw.bt = bt;
        nw.burst = bt;
        data[nw.idx] = nw;
        data_size++;
        //printf("Process idx: %d, burst: %d",nw.idx, data[nw.idx].burst);
    }
...
void create_process(int new_process) {
    ...
    if(proc_id == 0) {
        char nw_str[16];
        sprintf(nw_str, "%d", new_process);
        char** command = malloc(3 * sizeof(char *));
        for(int i=0; i < 3 ; i++) command[i] = malloc(128 * sizeof(char));
        command[0] = "./worker";
        command[1] = nw_str;
        command[2] = (char *)0;
        if(execvp(command[0], command) == -1) {printf("could not run worker!\n");}
    }
}
...
ProcessData find_next_process() {
  ...
  // location of next process in {data} array
	...
	for(int i=0; i < data_size; i++) {
        ...
        if(data[i].burst > 0 && data[i].at < smallest_time) {
            location = i;
            smallest_time = data[i].at;
        }
	}
...
void schedule_handler(int signum) {
    ...
    int wt, ct, tat; // waiting time, completion time, turnaround time.
    int at, bt, rt; // arrival time, burst time, response time,
    completion_time = total_time - arrival_time;
    tat = total_time - arrival_time;
    ...
    */
    check_burst();
    ProcessData nxt = find_next_process();
    ...
    if(ps[nxt.idx] == 0){
        ...
        */
        printf("Scheduler: Starting Process %d (Remaining Time: %d)\n", nxt.idx, data[nxt.idx].burst);
        create_process(nxt.idx);
    ...
    */
    else if (nxt.idx != running_process){
        // resume    
        running_process = nxt.idx;
